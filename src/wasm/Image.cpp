...
Image& Image::grayscale_avg() {
  if (channels < 3) {
    ...
    for(int i = 0; i < size; i += channels) {
      // (r + g + b) / 3
      int gray = (data[i] + data[i + 1] + data[i + 2]) / 3; memset(data + i, gray, 3); 
...
Image& Image::colour_mask(float r, float g, float b) {
  if (channels < 3) {
    printf("\e[31m[ERROR] Colour mask requires at least 3 channels, but this image has %d channels\e[0m\n", channels);
  } else {
    for (int i = 0; i < size; i += channels) {
      data[i] *= r;
      data[i + 1] *= g;
...
Image& Image::overlay_text(const char* txt, const Font& font, int x, int y, uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
